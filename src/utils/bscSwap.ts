...
export async function executeBSCSwap(params: SwapParams): Promise<SwapResult> {
  try {
    ...
    if (isTokenInBNB && !isTokenOutBNB) {
      // Swap native BNB for tokens - need to use swapExactETHForTokens
      // But first, we need to handle the fact that PancakeSwap expects native ETH/BNB
      // For native BNB, we use swapExactETHForTokens
